...
public class Map_Trans : MonoBehaviour
{
    ...
    public void TeleportCharacter()
    {
        ...
        {
            StartCoroutine(DelayedTeleport(targetObjects[index].targetTransform.position));
        }
    }
    ...
    IEnumerator DelayedTeleport(Vector3 targetPosition)
    {
        yield return new WaitForSeconds(0.5f); // 例如延迟0.5秒，你可以根据需要调整
        thirdPersonCharacter.transform.position = targetPosition;
    }
